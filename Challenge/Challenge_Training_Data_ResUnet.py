...
class PCG_Dataset(Dataset):
    def __init__(self, data, mask=None, pcg_transforms=None, mask_transforms=None):
        ...
        self.pcgs = sorted(os.listdir(self.train_data),
                           key=str)  # son todas las seniales del directrorio de train_data, ademas esta ordenada
        self.masks = sorted(os.listdir(self.train_masks),
                            key=str)  # son todas las mascaras del directrorio de train_masks, ademas esta ordenada
        print(self.pcgs)
        print(self.masks)
    ...
    def __getitem__(self, idx):
        pcg_name = os.path.join(self.train_data,
                                self.pcgs[idx])  # concatena la ruta con el nombre de la imagen en la posicion idx
    ...
    for epoch in range(resume_epoch, epochs):
        ...
        for mb, (x, y) in enumerate(train_loader, start=1):  # iteracion para cada mini batch (mb)
            ...
            y = y.to(device=device,
                     dtype=torch.long)  # para poder calcular la funcion de costo, recordemos que y tiene dimensiones 32,1,224,224. Tiene 1 canal y si mandamos un canal en la funcion de costo no lo va a tomar. entonces lo sacacamos para que quede 32,224,224. Quitamos la dimension 1
            ...
            train_predictions = torch.argmax(scores,
                                             dim=1)  # scores nos devuelve 3 canales. Nos quedamos con el canal que tiene mayor probabilidad. dim=1 es la dimension de los canales
            ...
            train_acc_mb_float = float(
                train_correct_num) / train_total  # cada 25 iteraciones calcula train_acc. Correctos sobre el total
...
train_acc_mb, train_cost_mb, val_acc_mb, val_cost_mb, dice_acc_mb, iou_acc_mb, test_acc_mb, test_cost_mb, test_dice_mb, test_iou_mb, train_acc_epoch, train_cost_epoch, val_acc_epoch, val_cost_epoch, dice_acc_epoch, iou_acc_epoch, test_acc_epoch, test_cost_epoch, test_dice_epoch, test_iou_epoch = train(
    model, optimiser_unet, start_epoch, epochs)
...
with torch.no_grad():
    scores = modelo(
        pcgs_val)  # calculo scores (tienen dos canales con la probabilidad que pertenezca a una u otra clase)
...
with torch.no_grad():
    scores = modelo(
        pcgs_test)  # calculo scores (tienen dos canales con la pribabilidad que pertenezca a una u otra clase)
    preds = torch.argmax(scores,
                         dim=1).float()  # calcula predicciones (como se queda con el mayor en la dimension de canales, da la mascara)
...
with torch.no_grad():
    scores = modelo(
        pcgs_prueba)  # calculo scores (tienen dos canales con la probabilidad que pertenezca a una u otra clase)
    preds = torch.argmax(scores,
                         dim=1).float()  # calcula predicciones (como se queda con el mayor en la dimension de canales, da la mascara)
